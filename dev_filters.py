...
def filterOutputFields(in_file, filtList, fExt):
    ...
    if arcpy.Exists(out_name):
        out_name = dev_f_utils.uniqName(out_name)
...
# ==============================================================
        ...
        dev_f_utils.addLogFields(out_file)
        # Write results to new FC
        with arcpy.da.UpdateCursor(out_file, dev_f_utils.fieldLists.logFields) as uCur:
             for row in uCur:
                # inline function for each featureID that is in the list
...
def filterOutputLog(in_file, filtList, fExt):
    ...
    out_name = path.basename(out_name)
    # Use this if running from python!!
#    out_name = homeDir+'\\'+path.basename(out_name)
    ...
    try:
        ...
        # Add filter fields
        dev_f_utils.addLogFields(logFile)

        # Write deletions to Log
        with arcpy.da.UpdateCursor(logFile, dev_f_utils.fieldLists.logFields) as uCur:
             for row in uCur:
                rowD = (item for item in filtList if item['fid'] == row[0]).next()
                row[20] = rowD['fname']
                row[21] = rowD['fparam']
...
def pop_filtList(in_file, filtList):
    ...
    try:
        with arcpy.da.SearchCursor(in_file, dev_f_utils.fieldLists.filterType, "", sr) as sCur:
            for row in sCur:
...
def main(in_file, newFields, type_list=None, lc_list=None, minTime=None, maxSpeed=None):
    ...
    # **********************************************************
    if type_list != dev_f_utils.filter_defaults.default_types:
        if type_list != None:
    ...
    if lc_list != dev_f_utils.filter_defaults.default_class:
        if lc_list != None:
            if len(filtList) == 0:
    ...
    if minTime > '0':
        ...
        import dev_redundant
        result = dev_redundant.filterRedundant(in_file, filtList, minTime, multi)
        fExt = fExt+'R'
        if not multi: multi = True
    ...
    if maxSpeed > '0':
        ...
        import dev_speed
        result = dev_speed.filterSpeed(in_file, filtList, maxSpeed, multi)
        fExt = fExt+'S'
...
#,"B1" "Biopsy Sample",
    defaults = dev_f_utils.parameters_from_args(defaults_tuple, sys.argv)
